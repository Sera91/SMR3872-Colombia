...
In this script we train a simple CNN on MNIST data. Instead of downloading the data we use the preloaded dataset from the torchvision repo.

Look at line 39 in the script:
```python
@profile
...
```bash
(Gabenv) $ sbatch job.slurm  # PLEASE edit your email address in job.slurm before submitting
```
