...
Optional: Learn how to conduct a [scaling analysis](https://researchcomputing.princeton.edu/support/knowledge-base/scaling-analysis) to find the optimal number of GPUs.
...
If you have a Pytorch version >1.0 use always `DistributedDataParallel`.
...
Torch distributed support 3 different backend API to communicate among node: NCCL, GLOO, MPI.
By default MPI is enabled only on the CPU, if you have not build by source Pytorch to detect MPI cuda-aware.

NOTE: One should prefer `nccl` over `gloo` since the latter will use TCP by first moving tensor to CPU.
...
The local rank is the rank assigned to each single process.
...
Below is a full Slurm script for using DDP on Leonardo where requesting only 2 GPUs per node:

([see script](job.slurm)).


The script above uses 2 nodes with 2 tasks per node and therefore 2 GPUs per node. This yields a total of 4 processes and each process can use 8 CPU-cores for data loading. An allocation of 4 GPUs is substantial so the queue time may be long. In all cases make sure that the GPUs are being used efficiently by monitoring the [GPU utilization] with nvprof.
...
If we want to ask for 4 GPUs per node we only need to modify the lines
