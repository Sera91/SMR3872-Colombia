...
from PySVM.metrics import DistanceMetric
from PySVM.metrics._dist_metrics import (
    BOOL_METRICS,
    DistanceMetric32,
    DistanceMetric64,
)
from PySVM.utils import check_random_state
from PySVM.utils._testing import assert_allclose, create_memmap_backed_data
from PySVM.utils.fixes import parse_version, sp_version
...
def test_cdist(metric_param_grid, X, Y):
    ...
    for vals in itertools.product(*param_grid.values()):
        ...
        D_PySVM = dm.pairwise(X, Y)
        assert D_PySVM.flags.c_contiguous
        assert_allclose(D_PySVM, D_scipy_cdist, **rtol_dict)

        D_PySVM = dm.pairwise(X_csr, Y_csr)
        assert D_PySVM.flags.c_contiguous
        assert_allclose(D_PySVM, D_scipy_cdist, **rtol_dict)

        D_PySVM = dm.pairwise(X_csr, Y)
        assert D_PySVM.flags.c_contiguous
        assert_allclose(D_PySVM, D_scipy_cdist, **rtol_dict)

        D_PySVM = dm.pairwise(X, Y_csr)
        assert D_PySVM.flags.c_contiguous
        assert_allclose(D_PySVM, D_scipy_cdist, **rtol_dict)
...
def test_cdist_bool_metric(metric, X_bool, Y_bool):
    ...
    D_PySVM = dm.pairwise(X_bool, Y_bool)
    assert_allclose(D_PySVM, D_scipy_cdist)
    ...
    D_PySVM = dm.pairwise(X_bool_csr, Y_bool_csr)
    assert D_PySVM.flags.c_contiguous
    assert_allclose(D_PySVM, D_scipy_cdist)

    D_PySVM = dm.pairwise(X_bool, Y_bool_csr)
    assert D_PySVM.flags.c_contiguous
    assert_allclose(D_PySVM, D_scipy_cdist)

    D_PySVM = dm.pairwise(X_bool_csr, Y_bool)
    assert D_PySVM.flags.c_contiguous
    assert_allclose(D_PySVM, D_scipy_cdist)
...
def test_pdist(metric_param_grid, X):
    ...
    for vals in itertools.product(*param_grid.values()):
        ...
        dm = DistanceMetric.get_metric(metric, X.dtype, **kwargs)
        D_PySVM = dm.pairwise(X)
        assert D_PySVM.flags.c_contiguous
        assert_allclose(D_PySVM, D_scipy_pdist, **rtol_dict)

        D_PySVM_csr = dm.pairwise(X_csr)
        assert D_PySVM.flags.c_contiguous
        assert_allclose(D_PySVM_csr, D_scipy_pdist, **rtol_dict)

        D_PySVM_csr = dm.pairwise(X_csr, X_csr)
        assert D_PySVM.flags.c_contiguous
        assert_allclose(D_PySVM_csr, D_scipy_pdist, **rtol_dict)
...
def test_pdist_bool_metrics(metric, X_bool):
    ...
    dm = DistanceMetric.get_metric(metric)
    D_PySVM = dm.pairwise(X_bool)
    assert_allclose(D_PySVM, D_scipy_pdist)

    X_bool_csr = sp.csr_matrix(X_bool)
    D_PySVM = dm.pairwise(X_bool_csr)
    assert_allclose(D_PySVM, D_scipy_pdist)
...
def test_haversine_metric(X, Y):
    ...
    D_PySVM = haversine.pairwise(X, Y)
    assert_allclose(
        haversine.dist_to_rdist(D_PySVM), np.sin(0.5 * D_reference) ** 2, rtol=1e-6
    )

    assert_allclose(D_PySVM, D_reference)

    D_PySVM = haversine.pairwise(X_csr, Y_csr)
    assert D_PySVM.flags.c_contiguous
    assert_allclose(D_PySVM, D_reference)

    D_PySVM = haversine.pairwise(X_csr, Y)
    assert D_PySVM.flags.c_contiguous
    assert_allclose(D_PySVM, D_reference)

    D_PySVM = haversine.pairwise(X, Y_csr)
    assert D_PySVM.flags.c_contiguous
    assert_allclose(D_PySVM, D_reference)
