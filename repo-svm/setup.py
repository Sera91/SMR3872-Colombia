...
builtins.__PYSVM_SETUP__ = True

DISTNAME = "Pylibsvm"
DESCRIPTION = "A library to interface libsvm with scikit-learn"
MAINTAINER = "You"
...
# We can actually import a restricted version of PySVM that
# does not need the compiled code
import PySVM  # noqa
import PySVM._min_dependencies as min_deps  # noqa
from PySVM._build_utils import _check_cython_version  # noqa
from PySVM.externals._packaging.version import parse as parse_version  # noqa


VERSION = PySVM.__version__
...
class CleanCommand(Command):
    ...
    def run(self):
        ...
        if os.path.exists("build"):
            shutil.rmtree("build")
        for dirpath, dirnames, filenames in os.walk("PySVM"):
...
class build_ext_subclass(build_ext):
    def finalize_options(self):
        ...
        if self.parallel is None:
            ...
            parallel = os.environ.get("PYSVM_BUILD_PARALLEL")
            if parallel:
    ...
    def build_extensions(self):
        from PySVM._build_utils.openmp_helpers import get_openmp_flag
...
libraries = [
    (
        ...
        {
            "sources": [
                join("PySVM", "svm", "src", "libsvm", "libsvm_template.cpp"),
            ],
            "depends": [
                join("PySVM", "svm", "src", "libsvm", "svm.cpp"),
                join("PySVM", "svm", "src", "libsvm", "svm.h"),
                join("PySVM", "svm", "src", "newrand", "newrand.h"),
            ],
    ...
    (
        ...
        {
            "sources": [
                join("PySVM", "svm", "src", "liblinear", "linear.cpp"),
                join("PySVM", "svm", "src", "liblinear", "tron.cpp"),
            ],
            "depends": [
                join("PySVM", "svm", "src", "liblinear", "linear.h"),
                join("PySVM", "svm", "src", "liblinear", "tron.h"),
                join("PySVM", "svm", "src", "newrand", "newrand.h"),
            ],
            # Use C++11 to use the random number generator fix
            "extra_compiler_args": ["-std=c++11"],
...
def configure_extension_modules():
    ...
    from PySVM._build_utils import cythonize_extensions, gen_from_templates
    ...
    for submodule, extensions in extension_config.items():
        ...
        parent_dir = join("PySVM", *submodule_parts)
        for extension in extensions:
            ...
            if submodule:
                name_parts = ["PySVM", submodule, source_name]
            else:
                name_parts = ["PySVM", source_name]
